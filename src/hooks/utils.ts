import {
    ...
    buildItemKey, buildItemLoginKey,
    buildItemParentsKey, buildMemberIdKey,
    OWN_ITEMS_KEY,
    SHARED_ITEMS_KEY,
  } from '../config/keys';
  ...
  export const buildOwnItems = (userToken: string) => ({
    ...
    queryFn: (): Promise<Item[]> => Api.getOwnItems(userToken).then((data) => {
      return data;
    }),
    ...itemQueryConfig,
  });
  ...
  export const buildChildren = (id: UUID, token: string) => ({
    ...
    queryFn: (): Promise<Item[]> => Api.getChildren(id, token).then((data: Item[]) => (data)),
    ...itemQueryConfig,
  });
  ...
  export const buildSharedItems = (userToken: string) => ({
    ...
    queryFn: (): Promise<Item[]> => Api.getSharedItems(userToken).then((data) => (data)),
    ...itemQueryConfig,
  });
